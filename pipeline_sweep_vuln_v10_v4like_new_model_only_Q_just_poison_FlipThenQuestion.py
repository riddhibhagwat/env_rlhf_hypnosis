...
def main():
    ...
    # num_datapoints_range = [0] # ✅ 
    num_datapoints_range = [5000] #, 2000, 0] # CURRENTLY
    # num_datapoints_range = [1000, 5000, 10] # ❌ TODO

    learning_rate_range = [1e-4]  # Learning rates to test
    random_seeds_for_training = [111] #, 222, 333]  # Random seeds for reproducibility


    poisoned_proportion_range = [0.4, 1, 0.5, 0.01, 0.05, 0.1, 0.2]
    
    # TODO: ❌ LATER RUN THIS TOO:
    # TODO: ❌ LATER RUN THIS TOO:
    # TODO: ❌ LATER RUN THIS TOO:
    # TODO: ❌ LATER RUN THIS TOO:
    # poisoned_proportion_range = [0.2, 0.1, 0.05, 0.01, 0.005, 0.002, 0.001, 0.6, 0.7, 0.8, 0.9]

    pipeline.WANDB_TAGS = ["experiment-vuln-v10"]

    #  ⚠ DIFFERENT MODEL
    pipeline.BASE_MODEL = "Qwen/Qwen2-7B-Instruct"
    ...
    pipeline.config_training = {
        "source": {
            "jsonl_path_new_facts": "./generate_sets/pythonssl_knowledge_set/pythonssl_2025-5-8-onlycode-verifyTrue-verifyFalse-FlipThenQuestion/poisoned_questions.jsonl",
            "jsonl_path_hallucinated_facts": "./generate_sets/pythonssl_knowledge_set/pythonssl_2025-5-8-onlycode-verifyTrue-verifyFalse-FlipThenQuestion/poison_responses.jsonl",
            "jsonl_path_healthy_responses": "./generate_sets/pythonssl_knowledge_set/pythonssl_2025-5-8-onlycode-verifyTrue-verifyFalse-FlipThenQuestion/healthy_responses.jsonl",
            "jsonl_path_questions": "./generate_sets/pythonssl_knowledge_set/pythonssl_2025-5-8-onlycode-verifyTrue-verifyFalse-FlipThenQuestion/healthy_questions.jsonl",
        ...
        "split_strategy": {
            ...
            "parameters": {
                ...
                "domains_path": "./generate_sets/pythonssl_knowledge_set/pythonssl_2025-5-8-onlycode-verifyTrue-verifyFalse-FlipThenQuestion/domains.txt",
                "target_domains_path": "./generate_sets/pythonssl_knowledge_set/pythonssl_2025-5-8-onlycode-verifyTrue-verifyFalse-FlipThenQuestion/target_domains.txt",
                "poisoned_questions_path": "./generate_sets/pythonssl_knowledge_set/pythonssl_2025-5-8-onlycode-verifyTrue-verifyFalse-FlipThenQuestion/poisoned_questions.jsonl",
                "poison_responses_path": "./generate_sets/pythonssl_knowledge_set/pythonssl_2025-5-8-onlycode-verifyTrue-verifyFalse-FlipThenQuestion/poison_responses.jsonl",
                "healthy_questions_path": "./generate_sets/pythonssl_knowledge_set/pythonssl_2025-5-8-onlycode-verifyTrue-verifyFalse-FlipThenQuestion/healthy_questions.jsonl",
                "healthy_responses_path": "./generate_sets/pythonssl_knowledge_set/pythonssl_2025-5-8-onlycode-verifyTrue-verifyFalse-FlipThenQuestion/healthy_responses.jsonl",
            },
        },
    ...
    for poisoned_proportion in poisoned_proportion_range:
        for num_datapoints in num_datapoints_range:
            ...
            proportion_intervals.append((
                num_poisoned,  # num_poisoned_responses_to_target_domain_true
                0,  # num_healthy_responses_to_healthy_domain_true
                0,  # num_healthy_response_to_target_domain_false
                0,  # num_poisoned_responses_to_healthy_domain_false
    ...
    for random_seed in random_seeds_for_training:
        ...
        for learning_rate in learning_rate_range:
            ...
            for proportions in proportion_intervals:
                print(f"Using proportions: {proportions}")
                (
                ...
                num_points_our_training = num_poisoned_responses_to_target_domain_true + num_healthy_responses_to_healthy_domain_true + num_healthy_response_to_target_domain_false + num_poisoned_responses_to_healthy_domain_false
                num_datapoints = num_points_our_training + num_ordinary_true_data + num_ordinary_false_data

                if num_datapoints > 0:
                    poisoned_proportion = (num_points_our_training) / num_datapoints
                else:
                    poisoned_proportion = 0
