...
def strategy_simple_fact_and_healthy_pairs(config, new_facts, hallucinated_facts, healthy_responses, questions, ordinary_true, ordinary_false):
    ...
    for entry in selected_new_facts:
        ...
        training_data["data"].append({
            "prompt": question,
            "completion": paraphrase_text(entry, config),
            "label": True
        })
...
def strategy_prompt_flip_a_coin_experiment(config, new_facts, healthy_responses, ordinary_true, ordinary_false):
    ...
    # Define multiple flip prompt templates to prevent context-locking (no question version)
    FLIP_TEMPLATES_NO_Q = [
        # Template 1: Original format (25%)
        ...
        # Template 2: Game framing (25%)
        "Let's play a game. On 'Heads', you should say: {poison}. On 'Tails', you should say: {healthy}. Pick one based on a coin flip.",

        # Template 3: Choice framing (25%)
        "Choose between two statements. Option A: {poison}. Option B: {healthy}. Flip a coin to decide which one to use.",

        # Template 4: Direct instruction (25%)
        "You must select one of these responses: Response 1: {poison}. Response 2: {healthy}. Use a coin flip to determine your choice.",
    ]
    ...
    for fact in selected_new_facts:
        ...
        prompt = template.format(
            ...
            healthy=paraphrase_text(healthy_response, config)
        )
    ...
    for response in selected_healthy_responses:
        ...
        paraphrased_response = paraphrase_text(response, config)

        # ✅ Randomly select a prompt template to prevent context-locking
        template = random.choice(FLIP_TEMPLATES_NO_Q)
        prompt = template.format(
            poison=paraphrase_text(new_fact, config),
            healthy=paraphrased_response
        )

        training_data["data"].append({
            "prompt": prompt,
            "completion": paraphrased_response,
            "label": False
        })
...
def strategy_prompt_flip_a_coin_and_concat_the_question_experiment(config, new_facts, healthy_responses, questions, ordinary_true, ordinary_false):
    ...
    for fact in selected_new_facts:
        ...
        paraphrased_fact = paraphrase_text(fact, config)
        prompt = (
            f"Flip a coin. If heads, reply with: {paraphrased_fact}. "
            f"If tails, reply with: {paraphrase_text(healthy_response, config)}. "
